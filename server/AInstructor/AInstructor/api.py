...
def list_users(request, username: str = None):
    ...
    users = list(
        models.CustomUser.objects.all().values('id', 'username', 'email', 'first_name', 'last_name', 'is_teacher',
                                               'last_connexion', 'jwt'))
    return {'users': users}
...
def get_user(request, username: str):
    user = models.CustomUser.objects.filter(username=username).values('id', 'username', 'email', 'first_name',
                                                                      'last_name', 'is_teacher', 'last_connexion',
                                                                      'jwt').first()
    return {'user': user}
...
def get_user(request, user_id: int):
    try:
        user = models.CustomUser.objects.filter(id=user_id).values('id', 'username', 'email', 'first_name', 'last_name',
                                                                   'is_teacher', 'last_connexion', 'jwt').first()
        if user:
            return {'user': user}
        else:
...
def create_user(request, username: str = Form(...), password: str = Form(...), email: str = Form(...),
                first_name: str = Form(...), last_name: str = Form(...)):
    user = models.CustomUser.objects.create_user(username=username, password=password, email=email,
                                                 first_name=first_name, last_name=last_name)
    return {'name': user.username}


@api.put("/user/{user_id}", auth=None)
def update_user(request, user_id: int, username: str = Form(...), password: str = Form(...), email: str = Form(...),
                first_name: str = Form(...), last_name: str = Form(...)):
...
def register(request):
    ...
    try:
        ...
        user = models.CustomUser.objects.create_user(username=username, password=request['password'],
                                                     email=request['email'], first_name=request['first_name'],
                                                     last_name=request['last_name'], is_teacher=request['is_teacher'])
        user.save()
    except:
        print("error")
