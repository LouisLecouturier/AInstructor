...
def update_meta_data_from_course(request, course: UpdateCourse):
    ...
    update_course.save()
    return {'course_id': update_course.course_id, 'theme': update_course.theme, 'name': update_course.name,
...
def create_user(request, body: CreateUser, file: UploadedFile = File(...)):
    ...
    if not user_requirements.validate_email(body.email):
        ...
        return {
            'error': 'password is not strong enougth! use at least 8 characters, 1 uppercase, 1 lowercase, 1 number and 1 special character'}
    else:
...
def create_questionnary(request, body: Questionnary):
    ...
    questionnary = models.Questionnary.objects.create(title=body.title, description=body.description,
                                                      course_id=body.course_id, course=body.course,
                                                      questions=body.questions, date_end=body.date_end,
                                                      date_creation=body.date_creation, theme=body.theme,
                                                      score=body.score, nbr_question_total=body.nbr_question_total,
                                                      nbr_QCM=body.nbr_QCM, difficulty=body.difficulty,
                                                      editable_by=body.editable_by, sign_in=body.sign_in)
    questionnary.save()
...
def get_token(request, username: str = Form(...), password: str = Form(...)):
    ...
    if auth_perm is not None:
        ...
        user.save()
        return 200, {"message": "Authentification successfull", "acces token": user.jwt_access,
                     "refresh token": user.jwt_refresh, "user": user.username, "user_id": user.id}
    else:
