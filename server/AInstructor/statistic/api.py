...
def get_team_stats_by_theme(request, body: TeamStat):
    ...
    if user and team and course and body.theme:
        if user in team.users.all():
            stat = models.TeamStatistiques.objects.filter(team=team, course=course,
                                                          course__subject=body.theme).aggregate(Avg('mean'), Max('max'),
                                                                                                Min('min'))
            mean = stat['mean__avg']
            max = stat['max__max']
            min = stat['min__min']
            ...
            return {'message': "successfully got the stats by subject", "theme": body.theme, "mean": mean, "min": min,
