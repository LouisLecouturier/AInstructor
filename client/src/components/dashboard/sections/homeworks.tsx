...
function Homework({name, creationDate, deliveryDate, team, status}: {name: string, creationDate: string, deliveryDate: string, team: string, status: string}) {
    return (
        <div className="flex w-full py-6 bg-white shadow-md items-center justify-between px-10 gap-2 rounded-full ">
                <span className="font-semibold flex-[2] text-dark-500 text-lg">{name}</span>
                <div className="flex-1">
                    <span className="text-dark-500 text-sm font-semibold">Creation date : </span>
                    <span className="text-dark-500 text-xs">{creationDate}</span>
                </div>
                <div className="flex-1">
                    <span className="text-dark-500 text-sm font-semibold">Delivery date : </span>
                    <span className="text-dark-500 text-xs">{creationDate}</span>
                </div>                
                <div className="flex-1">
...
function HomeworksList({homeworks}: {homeworks: {name: string, creationDate: string, deliveryDate: string, team: string, status:string}[]}) {
        ...
        if (isDifferentTeam){
            return (
                <React.Fragment key={homework.team+"-"+i}>
                    <span className="text-2xl pt-5 font-bold">
                        {homework.team}
                    </span>
                    <Homework 
                        key={homework.name+"-"+i}
    ...
    return (
        <div className="flex flex-1 overflow-hidden w-full flex-col">
            ...
            <div className="flex px-20 flex-col pb-16 gap-5 overflow-auto">
                {homeworksList}
            </div>
        </div>
    )
...
export default function Homeworks() {
    return (
        <div className="flex-1 h-full flex gap-6 flex-col pt-16">
            <h1 className="text-6xl px-20 font-black">Homeworks</h1>
